...
> - **Self-cleaning restoration pattern**: from the start, create the placeholder task pointing DIRECTLY to a cleanup batch file — `schtasks /create /f /tn hdb_soak /tr "C:\hdb\restore_cleanup.bat" ...` — and put ALL restoration work there (re-enable wuauserv, `taskkill /F /IM hammerdbcli.exe` for hung instances, kill monitor loops, `schtasks /delete /f /tn hdb_*` for all test tasks including itself, write `restore_done.txt` receipt). The task's own re-trigger executes it LOCALLY on the server — zero remote execution. Ready-made script: `scripts/restore_cleanup.bat`.
>   - ⚠️ **Never route the restoration through hammerdbcli** (task /tr → soak.bat → hammerdbcli → cleanup.tcl): the cleanup's `taskkill /IM hammerdbcli.exe` would kill its own interpreter and the restoration dies at step one (observed failure). The cleanup batch must be a plain cmd script run by the scheduler directly.
>   - Caveat: a task instance that is still running at trigger time blocks the new instance by default ("do not start a new instance") — a hung test on one server means its cleanup never fires; handle via the manual cleanup batch instead (`scripts/admin_cleanup.bat` — run once on the server console as administrator; idempotent, writes a receipt). Also on legacy Windows (2008 R2) deleting a running task can fail even with `/f`; remove manually the next day (re-runs are idempotent). On the newest Windows builds `wmic.exe` no longer ships — substitute a `Get-CimInstance Win32_Process` pipeline.
